#include "gpumemd/metal_backend.hpp"

#include <cassert>

int main() {
    if (!gpumemd::MetalBackend::is_available()) {
        return 0;
    }

    gpumemd::MetalBackend backend;
    const auto loaded = backend.load("metal_probe", 4096);
    assert(loaded.ok());
    assert(loaded.amount == 4096);
    assert(backend.is_loaded("metal_probe"));

    const auto snapshot = backend.snapshot();
    assert(snapshot.allocations.size() == 1);
    assert(snapshot.allocations[0].id == "metal_probe");
    assert(snapshot.allocations[0].bytes == 4096);

    const auto unloaded = backend.unload("metal_probe");
    assert(unloaded.ok());
    assert(unloaded.amount == 4096);
    assert(!backend.is_loaded("metal_probe"));
}
